...
spreadsheet_filename  <-  "IA1618_Inland_hills_mountains.xlsx" # Setting the filename to the spreadsheet containg data

# Reading in data, specifying which columns to read with the command 'range'
sheetname  <-  "IA1618" # Setting the name of the sheet within the spreadsheet containing data

av <- read_excel(spreadsheet_filename, sheet = sheetname, range = cell_cols("N:CA"), col_names = TRUE) #analysis variables, e.g. the 85 variables in the data; nod id's
av  <-  as.data.frame(av)

sheetname  <-  "OrdRes" # Setting the name of the sheet within the spreadsheet containing data
ordx <- read_excel(spreadsheet_filename, sheet = sheetname, range = cell_cols("N:P"), col_names = TRUE) #GNMDS
...
specresponse2 (av[ ,c(6, 8, 17, 18, 35, 37, 40, 52, 56, 65, 66)], main = " ", akse1, xlab = "GNMDS 1 (Coastal plains, outer- inner coast)")
  ...
  if(length(species) >= 1) {
    ...
    if(ismat == T) {
      ...
      if(bw == T){
        ...
        legend(0.70, 1.25, legend= c("Large river", "Landslide soil", "Mire", "Glacier", "Rugged terrain", "Mixed boreal forest", "Open areas with heathland", "Roads", "Reindeer husbandry facilities", "Arable land", "Surface cultivated land"), lty=1:ls, lwd = 2, col=1:ls, 
               bty = "n", cex = 0.85)
      }
    }
...
```
6, 8, 17, 18, 35, 37, 40, 52, 56, 65, 66

"Large river", "Landslide soil", "Mire", "Glacier", "Rugged terrain", "Mixed boreal forest", "Open areas with heathland", "Roads", "Reindeer husbandry facilities", "Arable land", "Surface cultivated land
...
specresponse2 (av[ ,c(6, 8, 17, 18, 34, 40, 46, 52,56,65,66)], main = " ", akse1_ia, model = "gam", xlab = "GNMDS 1 (Inland hills and mountains)")
